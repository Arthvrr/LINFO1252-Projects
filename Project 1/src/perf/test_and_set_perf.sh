...
echo "program_name,threads_number,compilation_time" > output_test_and_set.csv
...
for threads in "${mesures[@]}"; do
    ...
    for i in {1..5}; do
        ...
        elapsed=$({ time make -C "$build_path" test_and_set_main N=$(threads) > /dev/null; } 2>&1 | grep real | awk '{print $2}' | sed 's/[^0-9.]//g')
    ...
    echo "${program},${threads},${avg}" >> output_test_and_set.csv # Ajout des infos au CSV
done