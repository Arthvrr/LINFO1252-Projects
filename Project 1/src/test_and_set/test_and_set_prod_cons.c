#include "test_and_set/test_and_set_prod_cons.h"
...
void* produce(void* arg) {
    ...
    */
    ...
    while (true) {
        ...
        my_semaphore_wait(&is_empty); //on attends une place libre dans le buffer pour produire

        //SECTION CRITIQUE
        pthread_mutex_lock(&mutex);

        if (produced >= NUMBER_ELEMENTS) { //si le nombre d'éléments produits est plus grand ou égal au nombre d'éléments, on break
            //printf("Producer thread (ID: %ld) finished. Total produced: %d\n", (long)thread_id, produced);
            finished = 1;

            for (int i = 0; i < thread_cons; i++) { //on libère les threads consommateurs restants
        ...
        buffer[idx] = (int)pthread_self(); //cast vers un int
        produced++;
...
void* consume(void* arg) {
    ...
    */
    while (true){
        ...
        //si la production est terminée, il n'y a plus rien à consommer
        if (finished && consumed >= NUMBER_ELEMENTS) {
            //printf("Consumer thread %ld finished. Total consumed: %d\n", pthread_self(), consumed);
            pthread_exit(NULL); // Fin du thread consommateur
        }
...
int main(int argc, char* argv[]) {
    ...
    //initialise les sémaphores et mutex
