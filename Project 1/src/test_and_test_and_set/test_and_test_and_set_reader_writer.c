#include "test_and_test_and_set/test_and_test_and_set_reader_writer.h"


//2.5 Adaptation du 1.1 avec nos primitives d'attentes actives --> problème des lecteurs et écrivains
...
void* write_task(void* arg){
    ...
    while (true) {
        ...
        my_semaphore_wait(&resource_access);

        //écriture dans la section critique
        for (int i = 0; i < 10000; i++);

        my_semaphore_signal(&resource_access); //libère la ressource
    }
...
void* read_task(void* arg){
    ...
    while (true) {
        ...
        active_readers++;
        if (active_readers == 1) {
            my_semaphore_wait(&resource_access); //on bloque l'accès aux écrivains (prioritaires)
        }
        pthread_mutex_unlock(&mutex);
        ...
        active_readers--;
        if (active_readers == 0) {
            my_semaphore_signal(&resource_access); //on libère l'accès
        }
        pthread_mutex_unlock(&mutex);
    }
...
int main(int argc, char *argv[]){
    ...
    my_semaphore_init(&resource_access, 1);
    ...
    my_semaphore_destroy(&resource_access);
